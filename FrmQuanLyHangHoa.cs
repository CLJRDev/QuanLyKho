...
namespace QuanLyKho_Tuan1
{
    public partial class FrmQuanLyHangHoa : Form
    {
        ...
        private bool check()
        {
            ...
            status.Text = "Status: Lỗi cập nhập dữ liệu";
            return result;
        }

        private void loadDgvHangHoa()
        {
        ...
        private void but_them_Click(object sender, EventArgs e)
        {
            if (this.check() == false)
                return;
            string command = "exec spThemHangHoa @maHangHoa, @tenHangHoa";
            Dictionary<string, object> parameter = new Dictionary<string, object>();
            parameter.Add("@maHangHoa", txt_maHangHoa.Text);
            parameter.Add("@tenHangHoa", txt_tenHangHoa.Text);
            try
            {
                ...
                loadDgvHangHoa();
                status.Text = "Status: Cập nhập dữ liệu thành công";
            }catch(Exception ex)
            {
        ...
        private void but_sua_Click(object sender, EventArgs e)
        {
            if (this.check() == false)
                return;
            string command = "exec spSuaHangHoa @maHangHoa, @tenHangHoa";
            Dictionary<string, object> parameter = new Dictionary<string, object>();
            ...
            try
            {
                Database.Execute(command, parameter);
                loadDgvHangHoa();
                status.Text = "Status: Cập nhập hành hoá thành công";
            }catch(Exception ex)
            {
                MessageBox.Show(ex.Message, "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                status.Text = "Status: Lỗi cập nhập hàng hoá";
            }
        }
        ...
        {
            string command = "exec spXoaHangHoa @maHangHoa";
            Dictionary<string, object> parameter = new Dictionary<string, object>();
            parameter.Add("@maHangHoa", txt_maHangHoa.Text);
            parameter.Add("@tenHangHoa", txt_tenHangHoa.Text);
            try
            {
                Database.Execute(command, parameter);
                loadDgvHangHoa();
                status.Text = "Status: Xoá hàng hoá thành công";
            }
            catch(Exception ex)
            {
                MessageBox.Show(ex.Message, "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
