...
namespace QuanLyKho_Tuan1
{
    public partial class FrmThemPhieuNhap : Form
    {
        ...
        private bool checkPieuNhap()
        {
            ...
            {
                result = false;
                erroMes.SetError(txt_nguoiGiaoHang, "Loi dinh dang nguoi giao hang");
            }
            if(nguonNhap == "" || nguonNhap.Length > 100)
            {
                result = false;
                erroMes.SetError(txt_nguonNhap, "Loi dinh dang nguon nhap");
            }
        ...
        private bool checkChiTietPhieuNhap()
        {
            bool result = true;
            erroMes.Clear();
            if (String.IsNullOrEmpty(cbb_loaiHang.Text.Trim()))
            {
                erroMes.SetError(cbb_loaiHang, "Thieu loai hang");
                result = false;
            }
            if (String.IsNullOrEmpty(cbb_maHangHoa.Text.Trim()))
            {
                erroMes.SetError(cbb_maHangHoa, "Thieu hang hoa");
                result = false;
            }
            if (String.IsNullOrEmpty(cbb_maDonViTinh.Text.Trim()))
            {
            ...
            {
                erroMes.SetError(txt_donGia, "Don gia phai lon hon 0");
                result = false;
            }
            if(String.IsNullOrEmpty(txt_chietKhau.Text))
            {
                erroMes.SetError(txt_chietKhau, "Chieu khau phai lon hon 0");
                result = false;
            }
            if(String.IsNullOrEmpty(txt_soLuong.Text))
            {
                erroMes.SetError(txt_soLuong, "So luong phai lon hon 0");
                result = false;
            }
            status.Text = "Status: Loi cap nhap du lieu";
            return result;
        ...
        private void but_luu_Click(object sender, EventArgs e)
        {
            ...
            for(int i = 0; i < dgv_chiTietPhieuNhap.Rows.Count; i++)
            {
                ...
                parameter.Add("@soLuong", chiTietPhieuNhaps.Rows[i]["SoLuong"]);
                parameter.Add("@donGia", chiTietPhieuNhaps.Rows[i]["DonGia"]);
                parameter.Add("@chietKhau", chiTietPhieuNhaps.Rows[i]["ChietKhau"]);
