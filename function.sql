...
--------------------------------------------------------------------------------------------------
create function ufLayChiTietPhieuXuat(@maPhieuXuat varchar(3))
returns table
as return (select TenHangHoa,TenDonViTinh,SoLuong,DonGia,ChietKhau from ChiTietPhieuXuat inner join HangHoa on HangHoa.MaHangHoa = ChiTietPhieuXuat.MaHangHoa inner join DonViTinh on DonViTinh.MaDonViTinh = ChiTietPhieuXuat.MaDonViTinh where MaPhieuXuat = @maPhieuXuat)
--------------------------------------------------------------------------------------------------
create function ufLayChiTietPhieuNhap(@maPhieuNhap varchar(3))
returns table
...
returns table
as return select TenHangHoa,TenDonViTinh,SoLuong from ChiTietKho inner join Kho on Kho.MaKho = ChiTietKho.MaKho
				inner join DonViTinh on DonViTinh.MaDonViTinh = ChiTietKho.MaDonViTinh 
				inner join HangHoa on HangHoa.MaHangHoa = ChiTietKho.MaHangHoa where Kho.MaKho = @maKho